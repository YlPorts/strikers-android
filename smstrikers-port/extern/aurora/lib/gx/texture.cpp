...
namespace pre {
constexpr uint64_t ReadyBudgetBytes = 16ull * 1024ull * 1024ull;  // converted results waiting to be drawn
...
void enqueue(const GXTexObj_& obj) noexcept {
  ...
  // Bound expanded memory as well as compressed sources. A CMPR source can grow 8x.
  uint64_t expandedBytes = 0;
  for (u32 mip = 0; mip < mips; ++mip) {
    expandedBytes += uint64_t(std::max(obj.width() >> mip, 1u)) * std::max(obj.height() >> mip, 1u) * 4;
  }
  if (bytes == 0 || expandedBytes > 8ull * 1024ull * 1024ull) {
    return;
  }
...
uint64_t preconversion_ready_bytes_for_testing() noexcept {
  auto& st = pre::state();
  std::lock_guard lock{st.mutex};
  return st.parkedBytes;
}
