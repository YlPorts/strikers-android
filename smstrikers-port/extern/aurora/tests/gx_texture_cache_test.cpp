...
TEST_F(GxTextureCacheTest, MobileReadyBudgetIsBoundedAndShutdownReleasesIt) {
  std::vector<uint8_t> pixels(512 * 512 * 4);
  for (u32 id = 1; id <= 24; ++id) {
    pixels[0] = static_cast<uint8_t>(id);
    texture::preconvert_texture(make_texture(pixels.data(), id, GX_TF_RGBA8, 512, 512));
    ASSERT_TRUE(wait_for_preconversion());
    EXPECT_LE(texture::preconversion_ready_bytes_for_testing(), 16ull * 1024 * 1024);
  }
  EXPECT_GT(texture::preconversion_ready_bytes_for_testing(), 0);
  texture::shutdown();
  EXPECT_EQ(texture::preconversion_ready_bytes_for_testing(), 0);
  EXPECT_TRUE(texture::preconversion_idle_for_testing());
}

TEST_F(GxTextureCacheTest, LargeExpandedTextureSkipsBackgroundAllocation) {
  // Deliberately no source buffer: the size gate must run before any source copy.
  const uint8_t sentinel = 0;
  texture::preconvert_texture(make_texture(&sentinel, 1, GX_TF_CMPR, 2048, 2048));
  ASSERT_TRUE(wait_for_preconversion());
  EXPECT_EQ(testing::texture_conversions(), 0);
}
